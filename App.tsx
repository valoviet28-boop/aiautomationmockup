...
const App: React.FC = () => {
    ...
    const handleGenerateMockups = async () => {
        ...
        try {
            const images = await generateMockup(uploadedFile, mockupStyle, clothingType);
    ...
    return (
        <div className="min-h-screen bg-gray-900 text-gray-200 flex flex-col items-center p-4 sm:p-6 lg:p-8">
            ...
            <main className="w-full max-w-5xl mx-auto flex-grow">
                ...
                {uploadedFile && (
                    <div className="bg-gray-800 border border-gray-700 rounded-lg p-6 mb-8 flex flex-col md:flex-row gap-6 items-center">
                        <div className="flex-shrink-0">
                            <img src={uploadedFilePreview || ''} alt="Clothing preview" className="w-32 h-32 object-contain rounded-md bg-gray-700 p-1" />
                        </div>
                        <div className="flex-grow w-full space-y-4">
                            ...
                            <div>
                             ...
                             <div className="flex flex-col sm:flex-row gap-4 pt-2">
                                <button onClick={handleGenerateMockups} disabled={isLoading} className="w-full sm:w-auto flex items-center justify-center gap-2 bg-indigo-600 text-white font-bold py-3 px-6 rounded-lg hover:bg-indigo-500 transition-colors disabled:bg-gray-600 disabled:cursor-not-allowed">
                                    <SparklesIcon className="w-5 h-5" />
                                    Generate Mockups
                                </button>
                                <button onClick={() => setUploadedFile(null)} disabled={isLoading} className="w-full sm:w-auto bg-gray-600 text-gray-200 font-semibold py-3 px-6 rounded-lg hover:bg-gray-500 transition-colors disabled:opacity-50">
                                    Change Photo
                                </button>
                             </div>
                        </div>
